...
class GeneralDataset(data.Dataset):
  ...
  def prepare_img_input(self, image_np, box):    
    meta = Point_Meta(self.NUM_PTS, None, np.array(box), "", self.dataset_name)
  ...
  def check_data_all(self):
    ...
    for idx in range(self.length):
      img, heat_maps, mask, pts, torch_idx, torch_nopoints, ori_size = self.__getitem__(idx)

      img = img.permute(1, 2, 0).cpu().numpy()
      img = img * np.array([0.229, 0.224, 0.225]) + np.array([0.485, 0.456, 0.406])
      img = (img * 255.0).clip(0.0, 255.0).astype(np.uint8)
      cv2.imwrite(check_folder + "/%06d_img.png" % idx, cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

      heat_maps = heat_maps.cpu().numpy()   
      heat_map_num = heat_maps.shape[0] - 1
      for i in range(heat_map_num):
        if i == 0:
          heat_map = heat_maps[i + 1]
        else:
          heat_map += heat_maps[i + 1]      
      heat_map = (heat_map * 125.0).clip(0.0, 255.0).astype(np.uint8)
      heat_map_vis = cv2.applyColorMap(heat_map, cv2.COLORMAP_JET)
      cv2.imwrite(check_folder + "/%06d_heatmap.png" % idx, heat_map_vis)

      print(img.shape)
      print(heat_maps.shape)
      print(mask.shape)
      print(torch_idx)
      print(torch_nopoints)
      print(ori_size)      
